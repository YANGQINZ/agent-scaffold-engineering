...
public class HybridExecutor {
    ...
    public ChatResponse execute(AgentDefinition agentDef, ChatRequest request) {
        throw new AgentException(Constants.ErrorCode.AGENT_ORCHESTRATION_FAILED, "Hybrid编排功能待实现");
    }
    ...
    public Flux<StreamEvent> executeStream(AgentDefinition agentDef, ChatRequest request) {
        return Flux.error(new AgentException(Constants.ErrorCode.AGENT_ORCHESTRATION_FAILED, "Hybrid编排功能待实现"));
    }
