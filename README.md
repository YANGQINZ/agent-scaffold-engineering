...
| 研究分析 | AGENTSCOPE 引擎驱动多 Agent 顺序协作，完成深度研究任务 |
...
- **AGENTSCOPE 引擎**：基于 spring-ai-alibaba 的 SequentialAgent + ReactAgent，通过顺序编排多个 ReactAgent 子代理实现多 Agent 协作，每个子代理通过 outputKey 将输出存入共享状态（OverAllState），后续代理可通过模板变量引用前序结果
...
ChatFacade → TaskRuntime.execute()
    ...
    → routeAdapter(engineType)           # 路由至 EngineAdapter
        ...
        ├── AGENTSCOPE → AgentScopeAdapter  # SequentialAgent + ReactAgent 顺序编排
...
- **AGENTSCOPE 子节点**：委托给 AgentScopeAdapter，基于 SequentialAgent 顺序编排多个 ReactAgent，支持 MCP 工具调用
...
- **AGENTSCOPE 节点建议作为叶子节点**：HybridEngineAdapter 的流式优化仅对叶子节点启用 token 级流式（SequentialAgent.stream() 传播 ReactAgent 内部事件），中间节点同步执行发进度保活
